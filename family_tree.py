...
from family_tree_pb2 import FamilyMember, Relationships, FamilyUnit, FamilyTree
from utils_pb2 import EnglishDate, TraditionalDate, TamilMonth, TamilStar, Paksham, Thithi
...
class FamilyTree:
    ...
    def view_graph(self):
        ...
        def add_nodes_edges(node, graph):
            # Add node and its attributes, only display name and alive status
            label = f"<b>{node.name}</b><br>Alive: {node.data.get('alive_status', 'Unknown')}"
            graph.add_node(node.name, label=label, title=self.format_node_details(node), **node.get_all_attributes())
        ...
        nt.show("family_tree.html", notebook=False)
        print("Family tree graph saved to family_tree.html")
    
    def format_node_details(self, node):
        """Format node details for the tooltip."""
        details = f"<h2>{node.name}</h2>"
    ...
    def display_menu(self):
        while True:
            ...
            if choice == '1':
                ...
                name = input("Enter the name of the person: ")
                other_info = input("Enter the custom data for the person in key=value pairs (e.g. DOB=1990-01-01,Location=London, alive_status=True):")
                data = {}
                if other_info:
