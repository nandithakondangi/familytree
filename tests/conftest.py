...
def create_weasley_family_tree_proto():
    ...
    ron_rel = family_tree.relationships["RONAW"]  # noqa F841
    # No spouse or children for Ron in this basic example

    ginny_rel = family_tree.relationships["GINNW"]  # noqa F841
...
def weasley_handler(tmp_path):
    ...
    # Expected default output paths based on temp_dir_path for a new FamilyTreeHandler instance
    expected_default_html = test_dir / "family_tree.html"
    expected_default_data = test_dir / "family_tree_data.txtpb"
    ...
    handler = FamilyTreeHandler(
        temp_dir_path=str(test_dir), input_text_file=str(input_file)
    )
    # Yield handler and expected default paths for verification
    yield (
        handler,
        str(input_file),
        str(expected_default_html),
        str(expected_default_data),
    )
